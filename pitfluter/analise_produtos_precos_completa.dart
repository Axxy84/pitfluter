...
void main() async {
  ...
  try {
    ...
    final produtosPrecos = await supabase.from('produtos_precos').select('*');
    ...
    for (final tamanho in tamanhos) {
      ...
      final count =
          produtosPrecos.where((p) => p['tamanho_id'] == tamId).length;
    ...
    final produtos =
        await supabase.from('produtos').select('id, nome, tipo_produto');
    ...
    for (int i = 0; i < 10 && i < produtosPrecos.length; i++) {
      ...
      print(
          '${preco['id']?.toString().padLeft(2)} | ${produtoId?.toString().padLeft(10)} | ${produtoNome.toString().padRight(12)} | ${tamanhoId?.toString().padLeft(10)} | ${tamanhoNome.toString().padRight(12)} | ${preco['preco']}');
    }
    print('');
    ...
    final duplicatas =
        combinacoes.entries.where((entry) => entry.value > 1).toList();
    if (duplicatas.isEmpty) {
      ...
      for (final dup in duplicatas) {
        ...
        final tamanhoNome = tamanhosMap[tamanhoId]?['nome'] ?? 'N/A';
        print(
            '  - Produto: $produtoNome (ID: $produtoId) + Tamanho: $tamanhoNome (ID: $tamanhoId) = ${dup.value} registros');
      }
    }
    print('');
    ...
    final produtoTamanhosLista = produtoTamanhos.entries.map((entry) {
      ...
      return {
        ...
        'tamanhos': tamanhosSet
            .map((id) => tamanhosMap[id]?['nome'] ?? 'N/A')
            .join(', ')
      };
    }).toList();

    produtoTamanhosLista.sort((a, b) =>
        (b['qtd_tamanhos'] as int).compareTo(a['qtd_tamanhos'] as int));
    ...
    for (int i = 0; i < 20 && i < produtoTamanhosLista.length; i++) {
      ...
      print(
          '${item['id']?.toString().padLeft(2)} | ${item['nome'].toString().substring(0, item['nome'].toString().length > 20 ? 20 : item['nome'].toString().length).padRight(20)} | ${item['tipo'].toString().padRight(6)} | ${item['qtd_tamanhos']?.toString().padLeft(12)} | ${item['tamanhos']}');
    }
    print('');
    ...
    if (produtosSemPreco.isNotEmpty) {
      for (final produto in produtosSemPreco.take(10)) {
        print(
            '  - ID ${produto['id']}: ${produto['nome']} (${produto['tipo_produto']})');
      }
    ...
    print(
        'Produtos que NÃO estão em produtos_precos: ${produtosSemPreco.length}');
    print('Total de tamanhos cadastrados: ${tamanhos.length}');
    print(
        'Total de tamanhos usados em produtos_precos: ${tamanhosUnicos.length}');
    print('');
    ...
    final percentualPreenchimento =
        (realAtual / teoricoMaximo * 100).toStringAsFixed(2);
    ...
    for (final produto in produtos) {
      if (produto['ativo'] == true) {
        ...
        final produtoId = produto['id'] as int;
        final qtdPrecos =
            produtosPrecos.where((p) => p['produto_id'] == produtoId).length;

        if (!tiposProduto.containsKey(tipo)) {
          tiposProduto[tipo] = {'qtd_produtos': 0, 'qtd_precos': 0};
        }

        tiposProduto[tipo]!['qtd_produtos'] =
            tiposProduto[tipo]!['qtd_produtos']! + 1;
        tiposProduto[tipo]!['qtd_precos'] =
            tiposProduto[tipo]!['qtd_precos']! + qtdPrecos;
      }
    }
    ...
    tiposOrdenados.sort(
        (a, b) => b.value['qtd_precos']!.compareTo(a.value['qtd_precos']!));
    ...
    for (final entry in tiposOrdenados) {
      ...
      final media = dados['qtd_produtos']! > 0
          ...
          : '0.00';
      print(
          '${tipo.padRight(5)} | ${dados['qtd_produtos']?.toString().padLeft(12)} | ${dados['qtd_precos']?.toString().padLeft(10)} | ${media.padLeft(19)}');
    }
    print('');
    ...
    for (final entry in faixas.entries) {
      ...
      final media = quantidade > 0
          ...
          : '0.00';
      print(
          '${faixa.padRight(6)} | ${quantidade.toString().padLeft(10)} | ${media.padLeft(11)}');
    }
    ...
    print(
        '• Relação teórica: ${produtosUnicos.length} × ${tamanhosUnicos.length} = $teoricoMaximo');
    ...
    if (duplicatas.isEmpty && diferenca == 0) {
      ...
      print(
          '   ${produtosUnicos.length} produtos únicos × ${tamanhosUnicos.length} tamanhos = $teoricoMaximo registros');
      print('   Cada produto ativo tem um preço para cada tamanho disponível.');
    } else if (diferenca > 0) {
      print('⚠️  Temos MAIS registros que o esperado:');
      print(
          '   Esperado: ${produtosUnicos.length} × ${tamanhosUnicos.length} = $teoricoMaximo');
      print('   Real: $realAtual');
      print('   Diferença: +$diferenca registros extras');
      if (duplicatas.isNotEmpty) {
        print(
            '   Isso pode ser devido às ${duplicatas.length} duplicatas encontradas.');
      }
    } else {
