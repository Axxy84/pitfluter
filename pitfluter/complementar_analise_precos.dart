  ...
  try {
    ...
    for (final produtoId in precosPorProduto.keys.toList()..sort()) {
      ...
      for (final preco in precos) {
        ...
        print('  - Tamanho $tamanhoNome: R\$ $precoValor ${ativo ? '(ATIVO)' : '(INATIVO)'}');
      }
    }
    ...
    for (final categoria in categorias) {
      ...
      final produtosDaCategoria = produtos.where((p) => p['categoria_id'] == categoriaId).toList();
      final produtosComPreco = produtosDaCategoria.where((p) => precosPorProduto.containsKey(p['id'])).length;
    ...
    print('• Total de produtos ativos: ${produtos.where((p) => p['ativo'] == true).length}');
    print('• Total de produtos: ${produtos.length}');
    print('• Produtos com preços: ${precosPorProduto.keys.length}');
    print('• Produtos sem preços: ${produtos.length - precosPorProduto.keys.length}');
    ...
    print('• Teórico máximo (${produtosComPrecosCount} produtos × ${tamanhosCount} tamanhos): $teoricoMaximo');
    print('• Percentual de completude: ${(produtosPrecos.length / teoricoMaximo * 100).toStringAsFixed(1)}%');
